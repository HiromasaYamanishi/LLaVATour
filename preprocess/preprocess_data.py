...
class DataPreprocessor:
    ...
    def retrieve_sentences(
        ...
        if topk:
            save_path = f"../data/retrieved_{target}_top{k}.csv"
        else:
            save_path = f"../data/retrieved_{target}.csv"
        ...
        for spot_name in tqdm(self.text_image_df["spot_name"].unique()[59000:]):
            if spot_name in spot_names:
                continue
            df_tmp = self.text_image_df[self.text_image_df["spot_name"] == spot_name]
            spot_images = [
    ...
    def make_training_data_from_retrieved_review(
        ...
        for i, row in tqdm(retrieved_df.iterrows(), total=retrieved_df.shape[0]):
            ...
            if id in review_shorts:
                instructions.extend(
                    [
                        make_review_prompt(row["spot_name"], short=True),
                        make_review_context_prompt(
                            row["spot_name"], row["tag"], row["sex"], row["age"]
                        ),
                    ]
                )
    ...
    def make_training_data_from_retrieved_images(self, training_datas):
        ...
        retrieved_image_df = pd.read_csv(
            "/home/yamanishi/project/airport/src/analysis/LLaVA/data/retrieved_image_per_3text.csv",
            names=[
                "spot_name",
                "image_path",
                "review",
        ...
        for _, row in tqdm(
            ...
            instructions = [
                make_review_context_prompt(
                    spot_name, row["tag"], row["sex"], row["age"]
                )
            ]
            texts = [row["review"]]
    ...
    def make_training_data_from_posneg(self, training_datas):
        ...
        for _, row in tqdm(
            ...
            instructions = [
                make_review_context_posneg_prompt(
                    spot_name, row["tag"], row["sex"], row["age"], matches
                )
            ]
            texts = [row["review"]]
            training_datas.append(
