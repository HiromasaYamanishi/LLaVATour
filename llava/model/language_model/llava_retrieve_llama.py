...
from ..llava_arch import LlavaRetrieveMetaModel, LlavaRetrieveMMMetaModel, LlavaRetrieveMetaForCausalLM, LlavaRetrieveEmbMetaModel, LlavaRetrieveEmbMetaForCausalLM, LlavaRetrieveMMMetaForCausalLM #LlavaPOIMetaForCausalLM
...
class LlavaRetrievalEmbConfig(LlamaConfig):
    model_type = "llava_retrieval_emb"

class LlavaRetrievalMMConfig(LlamaConfig):
    model_type = "llava_retrieval_mm"
...
class LlavaRetrieveEmbModel(LlavaRetrieveEmbMetaModel, LlamaModel):
    config_class = LlavaRetrievalEmbConfig

    def __init__(self, config: LlamaConfig):
        super(LlavaRetrieveEmbModel, self).__init__(config)

class LlavaRetrieveMMModel(LlavaRetrieveMMMetaModel, LlamaModel):
    config_class = LlavaRetrievalMMConfig

    def __init__(self, config: LlamaConfig):
        super(LlavaRetrieveMMModel, self).__init__(config)
...
class LlavaRetrieveEmbForCausalLM(LlamaForCausalLM, LlavaRetrieveEmbMetaForCausalLM):
    config_class = LlavaRetrievalEmbConfig

    def __init__(self, config):
        super(LlamaForCausalLM, self).__init__(config)
        #print('llama model config', config)
        self.model = LlavaRetrieveEmbModel(config)
        #print('model', self.model)
        # for k,v in self.model.named_parameters():
        #     print(k, v)
        self.pretraining_tp = config.pretraining_tp
...
class LlavaRetrieveMMForCausalLM(LlamaForCausalLM, LlavaRetrieveMMMetaForCausalLM):
    config_class = LlavaRetrievalMMConfig

    def __init__(self, config):
        super(LlamaForCausalLM, self).__init__(config)
        #print('llama model config', config)
        self.model = LlavaRetrieveMMModel(config)
        #print('model', self.model)
        # for k,v in self.model.named_parameters():
        #     print(k, v)
        self.pretraining_tp = config.pretraining_tp
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        images: Optional[torch.FloatTensor] = None,
        start_entities: Optional[List[List[str]]] = None,
...
AutoConfig.register("llava_retrieval_emb", LlavaRetrievalEmbConfig)
AutoModelForCausalLM.register(LlavaRetrievalEmbConfig, LlavaRetrieveEmbForCausalLM)
