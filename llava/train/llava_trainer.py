...
from accelerate.utils import is_xpu_available, is_npu_available, is_mps_available,  is_torch_version
from typing import List, Optional
from enum import Enum
#from llava.train.train_retrieve import preprocess
...
def preprocess_v1(
    ...
    # print('conv', conv.roles)
    # print(sources[0])
    # print(sources[0]['from'])
    # print(conv.roles[0])
    conversations = []
    for i, source in enumerate(sources):
        if roles[source[0]["from"]] != conv.roles[0]:
        ...
        for j, sentence in enumerate(source):
            ...
            if not j%2:
                cur_prompt_index_ = cur_prompt_index + j//2
                retrieved_entities_tmp = []
                for retrieved_entity, prompt_index in zip(retrieved_entities, prompt_indices):
                    if prompt_index == cur_prompt_index_:
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, has_image=has_image, 
                             ...
                             cur_prompt_index=cur_prompt_index)
...
class LLaVATrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=False):
        print('save checkpoint')
        if 'count' not in vars(self):
            self.count = 0
        save_dir = os.path.join(self.args.output_dir, f'steps{int(self.count * self.args.save_steps)}')
        ...
        if self.args.lora_enable:
            ...
            # 標準のチェックポイント保存メソッドを呼び出す
            super(LLaVATrainer, self)._save_checkpoint(model, trial, metrics)
        self.count += 1
        
    def _save_checkpoint_(self, model, trial, metrics=None):
...
class LLaVARetrievalTrainer(LLaVATrainer):
        
    def _retrieval(self, model, inputs):
        #print('inputs', inputs)
        if hasattr(model, 'module'):
            model = model.module
        #model.get_model().retriever.eval()
        batched_data, batched_entity_index, batched_prompt_index = [], [], []

        count = 0
        for i, (prompts, start_entities, tasks) in enumerate(zip(inputs['prompts'], inputs['start_entities'], inputs['tasks'])):
            for prompt, start_entity, task in zip(prompts, start_entities, tasks):
                if start_entity=='sequential':continue
                batched_data.append((prompt, start_entity, task))
                batched_entity_index.append(i)
                batched_prompt_index.append(count)
                count+=1
    ...
    def _preprocess(self, inputs):
        # エンコードしていなかったテキストをencodeする
        input_ids, labels = [], []
        cur_prompt_index = 0
        for sources in inputs['sources']:
            #print('sources', sources)
            data_dict = preprocess(
                [sources],
                self.tokenizer,
                has_image = '<image>' in sources[0]['value'],
                retrieved_entities=inputs['retrieved_entities'],
                prompt_indices=inputs['prompt_indices'],
                cur_prompt_index = cur_prompt_index
            )
            cur_prompt_index += len(sources)//2
            input_ids.append(data_dict['input_ids'][0])
            labels.append(data_dict['labels'][0])
            data_dict['inputs_ids'] = data_dict['input_ids'][0]
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        """
        model.train()
        inputs = self._retrieval(model, inputs)
